...
//   const q50 = getQuantile(nums, 0.50);
...
//      - مرن: لو مراحل/مؤشرات محذوفة ما يطيح
//      ----------------------------- */
...
//         throw new Error("حط روابط CSV للسنة داخل src/sheets.js (Publish to web → CSV)");
//       }
...
//      - نقرأ الموجود في البيانات
//      - ونرتبه حسب PREFERRED_STAGE_ORDER إذا كانت موجودة
//      ========================================================= */
...
//   /* =========================================================
//      7) Helpers للقراءة المرنة للمؤشرات (لو انحذفت/ما موجودة)
//      ========================================================= */
//   const hasMetric = (metricName) => rows.some((r) => r.metric === metricName);
...
//   /* =========================================================
//      8) KPIs (مرنة)
//      - لو مؤشر غير موجود -> يرجع 0
//      - تقدر تخليها تختفي بدل 0 إذا تبين (كتبت لك تحت خيار)
//      ========================================================= */
//   const totals = useMemo(() => {
//     const words = sum(filteredRows.filter((r) => r.metric === "عدد الكلمات").map(pickByWeek));
//     const hours = sum(filteredRows.filter((r) => r.metric === "عدد الساعات").map(pickByWeek));
//     const people = sum(filteredRows.filter((r) => r.metric === "عدد الأشخاص").map(pickByWeek));
//     const videos = sum(filteredRows.filter((r) => r.metric === "مجموع مجموع الفيديوهات المصورة").map(pickByWeek));
//     return { words, hours, people, videos };
//   }, [filteredRows, week]);

//   // فيديوهات تظهر فقط إذا المرحلة = الكل أو التصوير + المؤشر موجود
//   const showVideosKPI = (stage === "الكل" || stage === "مرحلة التصوير") && hasMetric("مجموع مجموع الفيديوهات المصورة");

//   /* =========================================================
//      9) Line Chart: الكلمات عبر الأسابيع
//      + تلوين النقاط حسب القيمة
//      ========================================================= */
...
//   /* =========================================================
//      10) Bar Chart: مقارنة الأشخاص والساعات بين المراحل
//      - ديناميكي: يأخذ المراحل الموجودة فقط
//      - تلوين كل عمود حسب قيمته
//      ========================================================= */
//   const barPeopleHoursByStage = useMemo(() => {
//     const stagesOnly = dynamicStages.filter((s) => s !== "الكل");
//     const findRow = (st, metric) => rows.find((r) => r.stage === st && r.metric === metric);
...
//     }));
//   }, [rows, week, dynamicStages]); // dynamicStages لأن المراحل قد تتغير
...
//   /* =========================================================
//      11) Stacked Bar: الكلمات حسب المرحلة لكل أسبوع
//      - ديناميكي: يكوّن keys بناءً على المراحل الموجودة
//      - تلوين كل "مرحلة" بلون ثابت مبدئي (للوضوح)
//        * (لو تبينها حسب القيمة كذلك، أقدر أعدلها)
//      ========================================================= */
...
//     const weeks = ["week1", "w2", "w3", "w4"];
//     const findWordRow = (st) => rows.find((r) => r.stage === st && r.metric === "عدد الكلمات");
...
//   // ألوان ثابتة متكررة للمراحل في stacked (حتى لو عددهم أكثر من 3)
//   const stackedStageColors = useMemo(() => {
//     const palette = ["#059669", "#10b981", "#34d399", "#06b6d4", "#3b82f6", "#a855f7"];
//     const map = new Map();
//     stackedStagesOnly.forEach((st, idx) => map.set(st, palette[idx % palette.length]));
//     return map;
//   }, [stackedStagesOnly]);

//   /* =========================================================
//      12) Donut: توزيع الكلمات بين المراحل
//      - ديناميكي: فقط المراحل الموجودة
//      - تلوين حسب القيمة (أحمر/برتقالي/أزرق/أخضر)
//      ========================================================= */
...
//                     المصدر الحالي: <span className="font-semibold">{excelName}</span>
//                   </span>
//                 ) : (
...
//           <Card className="p-3">
//             <div className="flex items-center justify-between gap-3 flex-wrap">
//               <SectionTitle icon={Filter} title="الفلاتر" subtitle="السنة/المرحلة/الأسبوع" />
//               <div className="flex gap-3 flex-wrap items-end">
//                 <Select label="السنة" value={year} onChange={setYear} options={YEARS.length ? YEARS : ["2025"]} />

//                 {/* ✅ هنا التغيير: بدل STAGES الثابتة، صار dynamicStages */}
//                 <Select label="المرحلة" value={stage} onChange={setStage} options={dynamicStages} />

//                 <Select label="الأسبوع" value={week} onChange={setWeek} options={WEEKS} />
//               </div>
//             </div>
//           </Card>
//         </div>
...
//         {!hasData ? (
//           <Card className="p-6">
//             <div className="text-slate-700 font-semibold mb-2">لا توجد بيانات بعد</div>
//             <div className="text-sm text-slate-600 leading-6">
//               ✅ تأكد أنك فعلت <span className="font-semibold">Publish to web → CSV</span> لكل سنة،
//               وحطيت الروابط داخل <span className="font-semibold">src/sheets.js</span>.
//             </div>
//           </Card>
//         ) : null}
...
//           {/* لو تبين تخفين KPI إذا المؤشر غير موجود: لفّيه بشرط hasMetric */}
//           <KPI icon={Hash} label="إجمالي الكلمات" value={totals.words} hint={`${year} • ${stage} • ${week}`} />
//           <KPI icon={Clock3} label="إجمالي الساعات" value={totals.hours} hint={`${year} • ${stage} • ${week}`} />
//           <KPI icon={Users} label="عدد الأشخاص" value={totals.people} hint={`${year} • ${stage} • ${week}`} />

//           {showVideosKPI ? (
//             <KPI icon={Video} label="مجموع مجموع الفيديوهات المصورة" value={totals.videos} hint="التصوير فقط" />
//           ) : (
...
//                 </div>
//                 <div>
//                   <div className="text-sm text-slate-600">"مجموع الفيديوهات المصورة</div>
//                   <div className="text-sm text-slate-500">غير متوفر حالياً</div>
//                 </div>
//               </div>
//             </Card>
//           )}
...
//           <Card className="p-4">
//             <div className="mb-3">
//               <div className="text-base font-semibold text-slate-800">الكلمات عبر الأسابيع</div>
...
//                   <YAxis />
//                   <Tooltip />
//                   {/* لون الخط ثابت، والنقاط تتلوّن حسب القيمة */}
//                   <Line
//                     type="monotone"
//                     dataKey="value"
//                     stroke="#059669"
//                     strokeWidth={3}
//                     dot={({ cx, cy, payload }) => (
//                       <circle cx={cx} cy={cy} r={5} fill={colorLine(payload.value)} />
//                     )}
//                   />
//                 </LineChart>
//               </ResponsiveContainer>
...
//           <Card className="p-4">
//             <div className="mb-3">
//               <div className="text-base font-semibold text-slate-800">مقارنة الأشخاص والساعات بين المراحل</div>
...
//           <Card className="p-4">
//             <div className="mb-3">
//               <div className="text-base font-semibold text-slate-800">الكلمات حسب المرحلة لكل أسبوع</div>
...
//                   {/* ✅ هنا ديناميكي: يرسم Bar لكل مرحلة موجودة */}
...
//           <Card className="p-4">
//             <div className="mb-3">
//               <div className="text-base font-semibold text-slate-800">توزيع الكلمات بين المراحل</div>
...
//                 اختر <span className="font-semibold mx-1">المرحلة: الكل</span> لعرض التوزيع
//               </div>
//             ) : (
//               <div className="h-72">
//                 <ResponsiveContainer width="100%" height="100%">
...
//         <Card className="p-4">
//           <div className="mb-3">
//             <div className="text-base font-semibold text-slate-800">الجدول التفصيلي</div>
//             <div className="text-sm text-slate-500">المرحلة | المؤشر | week1 | w2 | w3 | w4 | الإجمالي</div>
//           </div>
...
//                   return (
//                     <tr key={idx} className="border-t border-emerald-100 hover:bg-emerald-50/40 transition">
//                       <td className="px-3 py-2 text-slate-800 whitespace-nowrap">{r.stage}</td>
//                       <td className="px-3 py-2 text-slate-700 whitespace-nowrap">{r.metric}</td>
...
//                       <td className="px-3 py-2 font-semibold text-slate-900">{formatNumber(total)}</td>
//                     </tr>
//                   );
//                 })}
//               </tbody>
...
export default function App() {
  ...
  async function loadFromSheet(selectedYear = year) {
    ...
    try {
      const url = SHEET_CSV_URLS[selectedYear];
      if (!url) throw new Error("رابط السنة غير موجود في src/sheets.js");

      const parsed = await loadRowsFromPublishedCSV(url);
      setRows(parsed);
      setExcelName(`Google Sheet ${selectedYear}`);

      // لو فلتر صار غير موجود بعد التحديث
      const stagesInFile = new Set(parsed.map((r) => r.stage));
      if (stage !== "الكل" && !stagesInFile.has(stage)) setStage("الكل");
    } catch (e) {
      setRows([]);
  ...
  /* =========================================================
     (E) Dynamic filters: stages / months / days
     ========================================================= */
  const dynamicStages = useMemo(() => {
    const set = new Set(rows.map((r) => r.stage).filter(Boolean));
    const ordered = [
  ...
  const dynamicMonths = useMemo(() => {
    const set = new Set(rows.map((r) => (r.month || "").trim()).filter(Boolean));
  ...
  const dynamicDays = useMemo(() => {
    let base = rows;
    if (month !== "الكل") base = base.filter((r) => (r.month || "").trim() === month);

    const set = new Set(base.map((r) => (r.day || "").trim()).filter(Boolean));
    const list = Array.from(set);
    ...
    list.sort((a, b) => {
      ...
      return bothNum ? na - nb : a.localeCompare(b, "ar");
    });

    return ["الكل", ...list];
  }, [rows, month]);

  // إذا تغيّر الشهر واليوم المختار ما عاد موجود -> ارجع "الكل"
  useEffect(() => {
    if (day !== "الكل" && !dynamicDays.includes(day)) setDay("الكل");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [month, dynamicDays]);

  /* =========================================================
     (F) Filtering rows by month/day/stage
     ========================================================= */
  const filteredRows = useMemo(() => {
    let out = rows;

    if (month !== "الكل") out = out.filter((r) => (r.month || "").trim() === month);
    if (day !== "الكل") out = out.filter((r) => (r.day || "").trim() === day);

    if (stage === "الكل") return out;
    return out.filter((r) => r.stage === stage);
  }, [rows, month, day, stage]);
  ...
  const totals = useMemo(() => {
    ...
    const hours = sum(filteredRows.filter((r) => r.metric === "عدد الساعات").map(pickByWeek));
    const people = sum(filteredRows.filter((r) => r.metric === "عدد الأشخاص").map(pickByWeek));
    const videos = sum(filteredRows.filter((r) => r.metric === "مجموع الفيديوهات").map(pickByWeek));
    return { words, hours, people, videos };
  }, [filteredRows, week]);

  const hasMetric = (metricName) => rows.some((r) => r.metric === metricName);
  const showVideosKPI =
    (stage === "الكل" || stage === "مرحلة التصوير") && hasMetric("مجموع الفيديوهات");

  /* =========================================================
     (H) Charts data (نفس منطقك + مرن للمراحل)
     ========================================================= */
  const lineWordsByWeek = useMemo(() => {
    const weeks = ["week1", "w2", "w3", "w4"];
    const wordRows = filteredRows.filter((r) => r.metric === "عدد الكلمات");
  ...
  const stagesOnly = useMemo(() => dynamicStages.filter((s) => s !== "الكل"), [dynamicStages]);
  ...
  const stackedStageColors = useMemo(() => {
    const palette = ["#059669", "#10b981", "#34d399", "#06b6d4", "#3b82f6", "#a855f7"];
    const map = new Map();
    stagesOnly.forEach((st, idx) => map.set(st, palette[idx % palette.length]));
    return map;
  }, [stagesOnly]);
    ...
    <div dir="rtl" className="min-h-screen bg-emerald-50/40">
      <div className="sticky top-0 z-20 backdrop-blur bg-white/80 border-b border-emerald-100">
        <div className="mx-auto max-w-7xl px-4 py-4 flex flex-col gap-3">
          ...
          <Card className="p-3">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <SectionTitle icon={Filter} title="الفلاتر" subtitle="السنة/الشهر/اليوم/المرحلة/الأسبوع" />
              <div className="flex gap-3 flex-wrap items-end">
                <Select label="السنة" value={year} onChange={setYear} options={YEARS.length ? YEARS : ["2025"]} />
                <Select label="الشهر" value={month} onChange={setMonth} options={dynamicMonths} />
                <Select label="اليوم" value={day} onChange={setDay} options={dynamicDays} />
                <Select label="المرحلة" value={stage} onChange={setStage} options={dynamicStages} />
                <Select label="الأسبوع" value={week} onChange={setWeek} options={WEEKS} />
              </div>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 space-y-6">
        {!hasData ? (
          <Card className="p-6">
            <div className="text-slate-700 font-semibold mb-2">لا توجد بيانات بعد</div>
            <div className="text-sm text-slate-600 leading-6">
              ✅ تأكد من Publish to web → CSV وحط الروابط داخل src/sheets.js
            </div>
          </Card>
        ) : null}

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPI icon={Hash} label="إجمالي الكلمات" value={totals.words} hint={`${year} • ${month} • ${day} • ${stage} • ${week}`} />
          <KPI icon={Clock3} label="إجمالي الساعات" value={totals.hours} hint={`${year} • ${month} • ${day} • ${stage} • ${week}`} />
          <KPI icon={Users} label="عدد الأشخاص" value={totals.people} hint={`${year} • ${month} • ${day} • ${stage} • ${week}`} />
          {showVideosKPI ? (
            <KPI icon={Video} label="عدد الفيديوهات" value={totals.videos} hint="التصوير فقط" />
          ) : (
            <Card className="p-4 opacity-60">
              <div className="flex items-center gap-3">
                <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-2">
                ...
                <div>
                  <div className="text-sm text-slate-600">عدد الفيديوهات</div>
                  <div className="text-sm text-slate-500">غير متوفر حالياً</div>
                </div>
              </div>
            </Card>
